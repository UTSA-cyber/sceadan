...
t.set_title(title)
t.set_footer(f)
t.append_head((a,b,c))  or [a,b,c]
t.append_data((a,b,c))
t.set_col_alignment(2,t.RIGHT)
t.latex_colspec = "a specific colspec that you like" e.g. (rr|ll)
print(t.typeset(mode='text/latex/html'))
...
def tvar(name,value,desc="",comma=False):
    ...
    if comma:
        value = commas(value,"%d")
    print("%s (%s): %s " % (name, desc, value))
    if desc=="": desc = name
