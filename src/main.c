...
typedef void (*input_f) ( const          char *const input_target,
...
{
    if(typeflag==FTW_F){
        process_file (fpath, ftw_callback_do_output, ftw_callback_file_type);
    }
    return 0;
}
            ...
            file_type_e      *const file_type )
{
    ...
    *input_target = argv[optind++];
    if(optind < argc){
        parse_arg (argv[optind],  &block_factor, parse_uint); /* set the block factor if one is provided */
    }
}
...
int main (const int argc, char *const argv[])
{
    ...
    parse_args (argc, argv,
		&input_target, &do_output, &do_input, &file_type );
    
    process_dir(input_target, do_output, file_type); /* if input_target is a file, it will be handled as a file */
    exit(0);
}
